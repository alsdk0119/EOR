...
function App() {
  ...
  async function handleLogin(loginName, password) {
    try {
      ...
      throw new Error(e.message || "로그인에 실패했습니다. 이름/비밀번호를 확인하세요.");
    }
  }
  ...
  const selectedRequests = useMemo(() => requests.filter((r) => r.status === "SELECTED"), [requests]);
  ...
  const calendarData = useMemo(() => {
    ...
    return buildMonthMatrix(year, month, selectedRequests, requests, users);
  }, [calendarMonth, selectedRequests, requests, users]);
  ...
  async function selectRequest(requestId) {
    ...
    setRequests((prev) => prev.map((r) => (r.id === requestId ? { ...r, status: "SELECTED" } : r)));
  ...
  return (
    <div className="app">
      ...
      <Routes>
        ...
        <Route path="/dashboard" element={<DashboardPage dashboard={dashboard} cancellations={cancellations} users={users} />} />
        <Route path="/calendar" element={<CalendarPage calendarMonth={calendarMonth} setCalendarMonth={setCalendarMonth} calendarData={calendarData} />} />
...
function MyRequestsPage({ myRequests, cancelRequest }) {
  ...
  return (
    <section className="card">
      ...
      <div className="table-wrap"><table><thead><tr><th>휴가일</th><th>유형</th><th>상태</th><th>신청시각</th><th>액션</th></tr></thead><tbody>{rows.map((r) => <tr key={r.id}><td>{r.leaveDate}</td><td>{leaveTypeLabel(r.leaveType)}</td><td>{statusLabel(r.status)}</td><td>{new Date(r.requestedAt).toLocaleString("ko-KR")}</td><td>{r.status === "APPLIED" ? <button onClick={() => cancelRequest(r.id)}>취소</button> : "-"}</td></tr>)}</tbody></table></div>
    </section>
  );
}

function DashboardPage({ dashboard, cancellations, users }) {
  return (
    <>
      <section className="card"><h2>휴가 사용내역 종합</h2><div className="kpi"><span>전체 {dashboard.total}</span><span>신청 {dashboard.applied}</span><span>취소 {dashboard.cancelled}</span></div></section>
...
function CalendarPage({ calendarMonth, setCalendarMonth, calendarData }) {
  return (
    <section className="card"><h2>관리자 달력(월간)</h2><div className="row"><label>월 선택 </label><input type="month" value={calendarMonth} onChange={(e) => setCalendarMonth(e.target.value)} /></div><div className="calendar">{["일", "월", "화", "수", "목", "금", "토"].map((d) => <div key={d} className="calendar-head">{d}</div>)}{calendarData.map((cell, idx) => <div key={`${cell.date}-${idx}`} className={`calendar-cell ${cell.inMonth ? "" : "muted"}`}><div className="calendar-date">{cell.day}</div>{cell.requestCount > 0 ? <div className="badge">신청 {cell.requestCount}</div> : null}{cell.applicants.map((item) => <div key={item.id} className={`selected-item type-${item.leaveType.toLowerCase()}`}>{typeFullLabel(item.leaveType)} {item.name}</div>)}</div>)}</div></section>
  );
}
  ...
  return (
    <>
      <section className="card">
        ...
        <div className="table-wrap"><table><thead><tr><th>간호사</th><th>휴가일</th><th>유형</th><th>신청시각</th><th>액션</th></tr></thead><tbody>{rows.map((r) => <tr key={r.id}><td>{users.find((u) => u.id === r.userId)?.name}</td><td>{r.leaveDate}</td><td>{leaveTypeLabel(r.leaveType)}</td><td>{new Date(r.requestedAt).toLocaleString("ko-KR")}</td><td className="row wrap"><button onClick={() => selectRequest(r.id)}>최종선정</button><button onClick={() => rejectRequest(r.id)}>미선정</button>{r.leaveType !== "GOLDKEY" ? <button onClick={() => addPriorityNote(r.id)}>협의메모</button> : null}</td></tr>)}</tbody></table></div>
      </section>
...
function buildMonthMatrix(year, month, selectedRequests, allRequests, users) {
